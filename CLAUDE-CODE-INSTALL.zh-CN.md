...
这份文档面向 Claude Code 使用者，说明如何把 `tool-first-agent` 接入 Claude Code，让 Claude Code 在写脚本之前优先检查和使用本机已有工具。

## 适用目标

接入后，Claude Code 在遇到下面这类任务时，应优先使用本机工具，而不是立即编写脚本：
...
Claude Code 不能像 Hermes 那样原生自动加载 `SKILL.md`。

因此接入方式是：

1. 把 `tool-first-agent` 安装到 Claude Code 可以访问的位置。
2. 在 Claude Code 的全局或项目规则中加入 Tool-First Rule。
3. 让 Claude Code 在相关任务开始前阅读并遵守 `SKILL.md`。
4. 需要时直接调用本仓库的脚本查询、检测、注册工具。
...
这种情况下，后面的路径把 `~/.agents/skills/tool-first-agent` 换成你的实际路径即可。

## 接入 Claude Code 全局规则

Claude Code 通常可以通过用户级或项目级 `CLAUDE.md` 添加规则。

推荐在用户级 `CLAUDE.md` 中加入下面内容，让所有项目都默认遵守：
...
如果你只想让某个项目使用这个规则，可以在项目根目录的 `CLAUDE.md` 中加入同样的规则。

适合场景：

- 某个项目经常处理文档、数据、媒体文件
- 项目有固定工具链，希望 Claude Code 优先复用
- 不想影响 Claude Code 在其他项目中的行为

## 验证 Claude Code 是否会使用

在 Claude Code 中发起一个测试任务，例如：

```text
请把这个 Markdown 文件转换成 HTML。开始前先检查本机是否已有合适工具，不要直接写脚本。
```

理想行为是 Claude Code 会先：
...
当 Claude Code 使用某个工具完成真实任务后，可以记录成功经验：
...
这样后续 Hermes、Claude Code 或其他 Agent 都可以通过 Hindsight 共享这条经验。

## 推荐工作流

Claude Code 遇到工具型任务时，应按这个顺序工作：
...
如果 Hermes 和 Claude Code 都使用 Hindsight 的 `default` bank，可以通过 tag 共享工具经验：
...
这样 Hermes 验证过的工具经验，Claude Code 可以回忆；Claude Code 验证过的工具经验，Hermes 也可以复用。
...
2. 在 Claude Code 的 `CLAUDE.md` 中加入：
...
这已经能让 Claude Code 在关键时刻知道要先使用 `tool-first-agent`。
